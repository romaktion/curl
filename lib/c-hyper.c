...
#define HYPER_ITER_ERROR 1 /* until defined by hyper.h */
                            ...
                            uint16_t http_status)
{
  ...
  curl_write_callback writeheader =
    data->set.fwrite_header? data->set.fwrite_header: data->set.fwrite_func;
  Curl_dyn_reset(&data->state.headerb);
  /* WARNING: this invents a fake status message */
  result = Curl_dyn_addf(&data->state.headerb, "HTTP/1.1 %03d OK\r\n",
                         (int)http_status);
  if(result)
    return result;
  len = Curl_dyn_len(&data->state.headerb);
                            ...
                            int select_res)
{
  ...
  *didwhat = KEEP_RECV;
  ...
  if(status_line(data, http_status))
    goto error;
